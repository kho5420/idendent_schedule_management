...
- 평일 전체휴진(진료 없음)인 날이 있는 경우
  - 그날을 그 주의 평일 휴무로 간주하여 재직 인원 전원을 '주차'로 표시한다. (명시적 연차는 연차로 유지)
  - 따라서 같은 주의 다른 평일에는 별도의 평일 휴무를 배정하지 않는다. (평일에 분배되던 주차가 전부 휴진일로 모인다)
  - is_weekday_fixed 직원은 평일 근무가 하루 줄어들므로 그 주 주말 1일을 대체 근무한다 (토요일 우선 근무). 위 8번 항목 참고.
