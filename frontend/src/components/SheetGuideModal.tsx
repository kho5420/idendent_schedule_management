...
const STEPS: GuideStep[] = [
    ...
    },
    {
        ...
        items: [
            ...
            '사용자 유형(User Type)에서 "외부(External)" 선택 → 만들기',
    ...
    {
        title: '4단계: 앱 게시 (모든 사용자 허용)',
        items: [
            '왼쪽 메뉴 "대상" 클릭',
            '"게시 상태" 섹션에서 "앱 게시" 버튼 클릭 → 확인 팝업에서 "확인" 클릭',
            '게시 상태가 "프로덕션 단계"로 바뀌면 완료',
            '※ 이 단계를 건너뛰면 본인 계정 외 다른 사람은 403 오류가 발생합니다',
            '※ 처음 로그인 시 "확인되지 않은 앱" 경고가 뜨면 "고급" → "안전하지 않은 페이지로 이동" 클릭',
        ],
    },
    {
        title: '5단계: OAuth 클라이언트 ID 발급',
        items: [
    ...
    {
        title: '6단계: 클라이언트 ID 앱에 입력',
        items: [
            '앱 메인 화면에서 "구글 스프레드시트" 카드를 선택하면 클라이언트 ID 입력란이 나타납니다',
            '5단계에서 발급받은 클라이언트 ID (xxxx.apps.googleusercontent.com)를 붙여넣고 "저장" 클릭',
